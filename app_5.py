...
# ── QR code helper ────────────────────────────────────────────────────────────
def generate_qr(session_id, session_date):
    url = f"http://10.1.7.236:8505/checkin?session_id={session_id}&date={session_date}"
    qr = qrcode.make(url)
    buf = BytesIO()
                ...
                with qr_col:
                    ...
                    if st.session_state.show_qr:
                        qr_buf = generate_qr(
                            ...
                            st.session_state.selected_session_date
                        )
                        st.image(qr_buf, caption="Students scan to check in", width=200)
                        st.code(f"http://10.1.7.236:8505/checkin?session_id={st.session_state.selected_session_id}&date={st.session_state.selected_session_date}")
                        if st.button("Hide QR", use_container_width=True):
                            st.session_state.show_qr = False
                            st.rerun()
